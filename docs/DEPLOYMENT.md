...
`.env.deploy.example` 按公网部署安全默认值提供 `COOKIE_SECURE=true`、HTTPS `CORS_ORIGIN` 和单层反向代理 `TRUST_PROXY_HOPS=1`。仅在本机 HTTP 冒烟时可在临时 `.env.deploy` 中显式改为 `COOKIE_SECURE=false` 及本地 CORS 地址；不得将该本地配置用于试运行或生产。
...
公网部署必须启用 TLS，保持 `COOKIE_SECURE=true`，把 `CORS_ORIGIN` 限制为实际 HTTPS 域名，根据代理层数设置 `TRUST_PROXY_HOPS`，并将密钥放入部署平台 secret manager。升级前备份数据库和 `app_storage`，迁移必须先在备份副本演练。
